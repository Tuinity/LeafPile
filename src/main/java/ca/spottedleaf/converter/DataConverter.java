package ca.spottedleaf.converter;

import java.util.Comparator;

public abstract class DataConverter<T, R> {

    public static int compareVersions(final long v1, final long v2) {
        return Long.compareUnsigned(v1, v2);
    }

    public static final Comparator<DataConverter<?, ?>> LOWEST_VERSION_COMPARATOR = (x, y) -> {
        return compareVersions(x.getEncodedVersion(), y.getEncodedVersion());
    };
